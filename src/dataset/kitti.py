...
class kitti(imdb):
  def __init__(self, image_set, data_path, mc):
    ...
    print ('_class_to_idx:',self._class_to_idx)
  ...
  def evaluate_detections(self, eval_dir, global_step, all_boxes):
    ...
    if self.mc.DATASET != 'KITTI':
      dt_dir = os.path.dirname(det_file_dir + '/data')
      val_txt = os.path.join(self._data_root_path, 'ImageSets',
                         self._image_set+'.txt')
      gt_dir = os.path.join(self._data_root_path, 'training/label_2')
      eval = Eval(self.mc.CLASS_NAMES,gt_dir,dt_dir)
      eval.evaluate_det()
      return 
    ...
    print ('cls:{} self._classes:{}'.format(cls,self._classes))
