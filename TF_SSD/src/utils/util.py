...
def batch_iou_v1(boxes,box):
    ious = []
    for box_ in boxes:
      ious.append(iou_v1(box_,box))
    return np.array(ious)

def batch_iou_ext(boxes,box):
    gt_bbox_center = np.array([bbox_transform_inv(box_tmp) for box_tmp in boxes])
    box_1= bbox_transform_inv(box)
    return batch_iou(gt_bbox_center,box_1)
...
def nms(boxes, probs, threshold):
  ...
  for i in range(len(order)-1):
    ovps = batch_iou_v1(boxes[order[i+1:]], boxes[order[i]])
