...
class imdb(object):
  ...
  def read_image_batch(self, shuffle=True):
    ...
    images, scales = [], []
    for i in batch_idx:
      print ('self._image_path_at({}):{}'.format(i,self._image_path_at(i)))
      ...
      #print ('mc.IMAGE_WIDTH:',mc.IMAGE_WIDTH)
      #print ('mc.IMAGE_HEIGHT:',mc.IMAGE_HEIGHT)
    #input('')
  ...
  def read_batch_gt_data(self,shuffle=True):
    mc = self.mc
    ...
    assert len(batch_idx) == mc.BATCH_SIZE
    ...
    for idx in batch_idx:
      ...
      for i in range(0,len(labels)):
        ...
        bbox[1] /=  orig_h
        bbox[3] /=  orig_h
        batch_gt_boxes[batch_id].append(bbox)
        ...
        assert bbox[0] < bbox[2] or bbox[1] < bbox[3]
      batch_id += 1
      
      '''
      batch_ids = np.ones((len(self._rois[idx]),1))*idx
      gt_data.append(np.concatenate([batch_ids,labels,gt_bbox],axis=1))
      '''
      
    return batch_gt_boxes,batch_gt_labels,batch_image
  ...
  def read_batch(self, shuffle=True):
    ...
    for idx in batch_idx:
      ...
      '''
      print ('=============image :',self._image_path_at(idx))
      label = [b[4] for b in self._rois[idx][:]]
      for i in range(0,len(label)):
        print ('[box:{} label:{}]'.format(bbox_transform(gt_bbox[i]),
                    self.mc.CLASS_NAMES[label[i]]))
      input('')
      '''
