...
class MutiBoxLossLayer():
    def __init__(self, mc):
        ...
        self.prior_boxes = tf.convert_to_tensor(mc.ANCHOR_BOX,dtype=tf.float32)
        self.prior_variances = tf.convert_to_tensor(mc.PRIORBOX_VARIANCES,dtype=tf.float32)
    ...
    def encode_bbox(self,input_mask_box,gt_boxes):

        mc = self.mc
        prior_width = self.prior_boxes[:,2] - self.prior_boxes[:,0]
        prior_height = self.prior_boxes[:,3] - self.prior_boxes[:,1]
        prior_center_x = (self.prior_boxes[:,0] + self.prior_boxes[:,2])/2.
        prior_center_y = (self.prior_boxes[:,1] + self.prior_boxes[:,3])/2.
        ...
        xmins = (bbox_center_x - prior_center_x) / prior_width / self.prior_variances[:,0]
        ymins = (bbox_center_y - prior_center_y) / prior_height / self.prior_variances[:,1]
        xmaxs = tf.log(bbox_width / prior_width) / self.prior_variances[:,2]
        ymaxs = tf.log(bbox_height / prior_height) / self.prior_variances[:,3]
    ...
    def cal_loc_loss(self,loc_data,gt_boxes,input_mask):
        ...
        self.encode_bbox(input_mask_box,gt_boxes)   
    ...
    def process(self,loc_data,conf_data,gt_boxes,gt_label,input_mask,all_match_overlaps):
        ...
        conf_data_norm = tf.reshape(conf_data,[mc.BATCH_SIZE, mc.ANCHORS_NUM,mc.CLASSES])
        ...
        self.cal_loc_loss(loc_data,gt_boxes,input_mask)
